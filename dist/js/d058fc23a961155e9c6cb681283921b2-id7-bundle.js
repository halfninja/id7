...
function () {
  ...
  _createClass(Navigation, [{
    ...
    value: function wireEventHandlers() {
      ...
      jquery__WEBPACK_IMPORTED_MODULE_3___default()(window).on('hashchange', jquery__WEBPACK_IMPORTED_MODULE_3___default.a.proxy(this.hashChanged, this)); // Accessibility

      this.$container.on('keydown', function (ev) {
        var $focus = jquery__WEBPACK_IMPORTED_MODULE_3___default()(document.activeElement);
        var $li = $focus.parent();
        var isEnterOrDown = ev.keyCode === 40 || ev.keyCode === 13;
        var isEnterOrUp = ev.keyCode === 38 || ev.keyCode === 13; // Is there a Bootstrap method to test this?

        var dropdownOpen = $li.find('.dropdown-menu').parent().hasClass('open'); // Necessary for escape/clear to work to close the menu
        ...
        if ($li.hasClass('dropdown') && (isEnterOrDown && !dropdownOpen || isEnterOrUp && dropdownOpen)) {
          ...
          ev.preventDefault();
          var $elementToFocus = dropdownOpen ? $li.next().children().first() // next item (debatable to do this..)
        ...
        if (arrowRight && primaryNavItemInFocus) {
          ...
          // Move to the right hand nav element, opening its dropdown and closing this one in the process
          //                           li.dropdown
          var nextNav = $li.parents().eq(1).next();

          if (nextNav.hasClass('dropdown')) {
            $li.parents().eq(1).find('a').attr('data-toggle', 'dropdown'); // close ours
...
jquery__WEBPACK_IMPORTED_MODULE_3___default()(function () {
  jquery__WEBPACK_IMPORTED_MODULE_3___default()('.id7-navigation').id7Navigation();
  ...
  jquery__WEBPACK_IMPORTED_MODULE_3___default()('.nav-tabs a').on('shown.bs.tab.id7Navigation', function (e) {
    return Object(_change_location_hash__WEBPACK_IMPORTED_MODULE_5__["default"])(e.target.hash);
  });
});
