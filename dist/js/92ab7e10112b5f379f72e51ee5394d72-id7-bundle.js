  ...
  _createClass(Navigation, [{
    ...
    value: function wireEventHandlers() {
      ...
      this.$container.on('keydown', function (ev) {
        ...
        var $li = $focus.parent();
        var isEnterOrDown = ev.key === 'ArrowDown' || ev.key === 'Enter';
        var isEnterOrUp = ev.key === 'ArrowUp' || ev.key === 'Enter';
      ...
      jquery__WEBPACK_IMPORTED_MODULE_3___default()('.dropdown-menu').each(function (i, el) {
        ...
        if ($linkElement.length > 0 && $linkElement.attr('data-toggle') !== 'dropdown') {
          ...
          $linkElement.click(function (ev) {
            window.location = $linkElement.attr('href');
            ev.stopPropagation();
            return false;
          }); // we need to special case clicks on the trigger because we use hovers to open up the menu
        }
      });
    }
    /**
