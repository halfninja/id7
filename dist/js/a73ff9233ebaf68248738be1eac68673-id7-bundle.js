    ...
    value: function wireEventHandlers() {
      ...
      this.$container.on('keydown', function (ev) {
        ...
        if ($li.hasClass('dropdown') && (isEnterOrDown && !dropdownOpen || isEnterOrUp && dropdownOpen)) {
          ...
          ev.preventDefault(); // const $elementToFocus = dropdownOpen ? $li.next().children().first() : $li.find('.dropdown-menu').find("li:first-child a");
        ...
        if (arrowRight && $focus.parent().parent().hasClass("navbar-nav")) {//a
        }
      });
    }
  }], [{
