...
function () {
  ...
  _createClass(Navigation, [{
    ...
    value: function wireEventHandlers() {
      ...
      this.$container.on('keyup', function (ev) {
        console.log(ev.keyCode);
        var $focus = jquery__WEBPACK_IMPORTED_MODULE_3___default()(document.activeElement);

        if ($focus.parent().hasClass('dropdown') && ev.keyCode === 40 && $focus.parent().find('.dropdown-menu').is(':hidden')) {
          $focus.parent().find('.dropdown-menu').dropdown('toggle');
        }
      });
    }
  }], [{
