  ...
  _createClass(Navigation, [{
    ...
    value: function wireEventHandlers() {
      ...
      this.$container.on('keydown', function (ev) {
        ...
        } // Allow opening and closing the focussed dropdown with up/down


        if ($li.hasClass('dropdown') && (isEnterOrDown && !dropdownOpen || isEnterOrUp && dropdownOpen)) {
          $li.find('.dropdown-menu').dropdown('toggle');
          ev.preventDefault();
          var $elementToFocus = dropdownOpen ? $li.next().children().first() : $li.find('.dropdown-menu').find('li:first-child a');
          $elementToFocus.focus();
          return false;
        }

        var arrowRight = ev.keyCode === 39;
        var primaryNavItemInFocus = $focus.parent().parent().hasClass('navbar-nav');
